...
while cap.isOpened():  # --> --> True: <-- <--
    ...
    for face_coordinates, fname in zip(faces, face_name):
        ...
        df = pd.DataFrame(data, columns=['face_name', 'emotion_probability', 'emotion_mode', 'Attendance', 'Time'])
        df.to_csv(r'C:\Users\admin\PycharmProjects\SPAAC\file.csv', mode='a', index=False, header=False)
        print(df)
