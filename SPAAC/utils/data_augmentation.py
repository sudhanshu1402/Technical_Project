...
class ImageGenerator(object):
    ...
    def _do_random_crop(self, image_array):
        ...
        scale_factor = np.random.uniform(self.zoom_range[0], self.zoom_range[1])
    ...
    def contrast(self, image_array):
        gray_scale = self._gray_scale(image_array).mean() * np.ones_like(image_array)
    ...
    def lighting(self, image_array):
        covariance_matrix = np.cov(image_array.reshape(-1, 3) / 255.0, rowvar=False)
    ...
    def transform(self, image_array, box_corners=None):
        ...
        if self.horizontal_flip_probability > 0:
            image_array, box_corners = self.horizontal_flip(image_array, box_corners)

        if self.vertical_flip_probability > 0:
            image_array, box_corners = self.vertical_flip(image_array, box_corners)
        return image_array, box_corners
            ...
            for key in keys:
                ...
                if mode == "train" or mode == "demo":
                    if self.ground_truth_transformer != None:
                        ...
                        ground_truth = self.ground_truth_transformer.assign_boxes(
                            ground_truth
                        )
                    else:
                ...
                if self.grayscale:
                    image_array = cv2.cvtColor(
                        image_array.astype("uint8"), cv2.COLOR_RGB2GRAY
                    ).astype("float32")
