...
def make_mosaic(images, num_rows, num_cols, border=1, class_names=None):
    ...
    for image_arg in range(num_images):
        ...
        mosaic[
            row * paddedh : row * paddedh + image_shape[0],
            col * paddedw : col * paddedw + image_shape[1],
        ] = image
    return mosaic
...
def draw_mosaic(
    data, num_rows, num_cols, class_vectors=None, class_decoder=None, cmap="gray"
):
...
if __name__ == "__main__":
    ...
    pretty_imshow(
        plt.gca(), make_mosaic(kernel_conv1_weights, box_size, box_size), cmap=cm.binary
    )
    plt.show()
